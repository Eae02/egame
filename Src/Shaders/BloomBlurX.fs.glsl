...
layout(binding=1) uniform texture2D lowerLevel;
...
void main()
{
	...
	{
		color += texture(sampler2D(inputImage, imageSampler), texCoord + vec2(pixelSize.x * i * 2, 0)).rgb * kernel[i];
		color += texture(sampler2D(inputImage, imageSampler), texCoord - vec2(pixelSize.x * i * 2, 0)).rgb * kernel[i];
	}
	
	color += texture(sampler2D(lowerLevel, imageSampler), texCoord).rgb;
