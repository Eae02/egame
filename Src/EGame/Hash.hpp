...
{
public:
	constexpr CTStringHash(uint32_t _hash = 0) noexcept : hash(_hash) {}

	constexpr CTStringHash(const char* const text) noexcept : hash(CalcHash(text, SIZE_MAX, FNV_OFFSET_BASIS)) {}
...
private:
	...
	static constexpr uint32_t CalcHash(const char* const str, const size_t len, const uint32_t value) noexcept
	{
		return (len == 0 || *str == '\0')
		           ? value
		           : CalcHash(str + 1, len - 1, (value ^ static_cast<uint32_t>(str[0])) * FNV_PRIME);
	}
};
} // namespace eg
