...
namespace eg
{
	...
	public:
		...
		public:
			...
			void StepOver()
			{
				m_index += m_timers[m_index].totalChildren;
			}
			
			const std::string& CurrentName() const
			{
				return m_timers[m_index].name;
			}
			
			float CurrentValue() const
			{
				return m_timers[m_index].timeNS;
			}
			
			int CurrentDepth() const
			{
				return m_timers[m_index].depth;
			}
		...
		TimerCursor GetCPUTimerCursor() const
		{
			return TimerCursor(m_cpuTimers);
		}
		
		TimerCursor GetGPUTimerCursor() const
		{
			return TimerCursor(m_gpuTimers);
		}
