...
void eg::detail::WebDownloadAssetPackages(std::function<void()> onComplete) { onComplete(); }
std::istream* eg::detail::WebGetDownloadedAssetPackageStream(std::string_view name) { return nullptr; }
...
namespace eg
{
	...
	{
		auto endIt = std::remove_if(assetBinaries.begin(), assetBinaries.end(), [&] (const auto& b) { return b.freeAfterInit; });
	...
	{
		WriteAssetsToCache(
			std::span<const char>(fetch->data, fetch->numBytes),
	...
	static std::unique_ptr<CachedAssetBinary> TryLoadCachedAssets(const DownloadAssetPackageArgs& args)
	{
		...
		{
			CacheBeginPrint(args) << "Version mismatch (got '" << cachedId << "' expected '" << args.cacheID << "')" << std::endl;
			return nullptr;
		}
		cachedAssetsIdStream.close();
		...
		{
			CacheBeginPrint(args) << "Package corrupted " << std::hex << *reinterpret_cast<uint32_t*>(magic) << std::dec << std::endl;
			return nullptr;
		}
	...
	void detail::WebDownloadAssetPackages(std::function<void()> onComplete)
	{
		...
		bool anyPackageCache = std::any_of(assetPackagesToDownload.begin(), assetPackagesToDownload.end(),
			[&] (const auto& p) { return !p.cacheID.empty(); });
		
		if (anyPackageCache)
		{
			EM_ASM(
				FS.mkdir('/asset_cache');
				FS.mount(IDBFS, {}, '/asset_cache');
				FS.syncfs(true, function (err) {
					Module.cwrap("AssetCacheCreated", "", [])();
				});
			);
		}
		else
...
extern "C" void EMSCRIPTEN_KEEPALIVE AssetCacheCreated() { eg::FetchNextAssetPackage(); }
