...
public:
	...
	explicit ModelAssetWriter(std::ostream& stream) : m_stream(&stream) { BinWrite(*m_stream, V::Name.hash); }

	void WriteMesh(
		std::span<const V> vertices, std::span<const uint32_t> indices, std::string_view name, MeshAccess access,
		const Sphere& boundingSphere, const AABB& boundingBox, std::string_view materialName = {})
	{
	...
	void End() { detail::ModelAssetWriterEnd(*m_stream, Skeleton(), {}); }
