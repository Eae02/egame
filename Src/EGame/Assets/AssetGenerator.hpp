...
{
public:
	AssetGenerateContext(
		std::string_view currentDir, std::string_view assetName, const YAML::Node& node, const YAML::Node& rootNode)
		: m_currentDir(currentDir), m_assetName(assetName), m_node(&node), m_rootNode(&rootNode)
	{
	}

	const YAML::Node& YAMLNode() const { return *m_node; }

	const YAML::Node& RootYAMLNode() const { return *m_rootNode; }

	std::string ResolveRelPath(std::string_view relPath) const { return Concat({ m_currentDir, relPath }); }
	...
	void AddLoadDependency(std::string relPath) { m_loadDependencies.push_back(std::move(relPath)); }

	std::string_view AssetName() const { return m_assetName; }

	std::string RelSourcePath() const;

	const std::vector<std::string>& FileDependencies() const { return m_fileDependencies; }
