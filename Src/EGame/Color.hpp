...
struct ColorLin : Color
{
	...
	explicit ColorLin(const Color& c) : Color(c) {}

	inline ColorLin(const struct ColorSRGB& c);

	ColorLin(float _r, float _g, float _b, float _a = 1.0f) : Color(_r, _g, _b, _a) {}

	ColorLin ScaleRGB(float scale) const { return ColorLin(r * scale, g * scale, b * scale, a); }

	ColorLin ScaleAlpha(float scale) const { return ColorLin(r, g, b, a * scale); }
...
struct ColorSRGB : Color
{
	...
	explicit ColorSRGB(const Color& c) : Color(c) {}

	inline ColorSRGB(const struct ColorLin& c);

	ColorSRGB(float _r, float _g, float _b, float _a = 1.0f) : Color(_r, _g, _b, _a) {}

	ColorSRGB ScaleAlpha(float scale) const { return ColorLin(r, g, b, a * scale); }
...
inline ColorLin::ColorLin(const ColorSRGB& c) : Color(SRGBToLinear(c.r), SRGBToLinear(c.g), SRGBToLinear(c.b), c.a) {}
} // namespace eg
