...
public:
	...
	uint32_t NumBones() const { return static_cast<uint32_t>(m_bones.size()); }

	bool HasUniqueBoneNames() const { return m_hasUniqueBoneNames; }

	std::optional<uint32_t> ParentId(uint32_t boneId) const;

	uint32_t DualId(uint32_t boneId) const { return m_bones[boneId].dual; }

	const glm::mat4& InverseBindMatrix(uint32_t boneId) const { return m_bones[boneId].inverseBindMatrix; }
