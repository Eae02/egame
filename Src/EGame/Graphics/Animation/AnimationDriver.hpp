...
namespace eg
{
	...
	public:
		...
		explicit AnimationDriver(const Model& model)
			: m_model(&model) { }
		
		void PlayLoop(std::string_view name, int channel, float transitionTime = 0.3f,
		...
		bool Ready() const
		{
			return m_targetMatrices != nullptr && m_model;
		}
		...
		std::shared_ptr<const glm::mat4[]> BoneMatricesSP() const
		{
			return m_targetMatrices;
		}
#endif
	...
	private:
		struct ActiveAnimation
		{
			...
			void ModulateTime()
			{
				m_time = std::fmod(m_time, m_animation->Length());
			}
