	...
	for (auto it = m_channels.begin(); it != m_channels.end();)
	{
		...
		if (channel.m_next.IsActive())
		{
			...
			if (!channel.m_previous.IsActive() && pastEnd)
			{
				...
				channel.m_nSpeedScale =
					channel.m_current.m_animation->Length() / channel.m_previous.m_animation->Length();
			}
		}
	...
	for (uint32_t i = 0; i < numTargets; i++)
	{
		...
		for (const auto& channelP : m_channels)
		{
			...
			if (channel.m_previous.IsActive())
			{
				...
				float nt = std::fmod(
					channel.m_transitionProgress * channel.m_nSpeedScale, channel.m_current.m_animation->Length());
...
void AnimationDriver::ActiveAnimation::Play(
	const Animation& animation, bool looping, bool mirrorLR, std::function<void()> endCallback)
{
