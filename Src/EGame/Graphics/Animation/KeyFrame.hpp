...
namespace eg
{
	...
	struct KeyFrame
	{
		...
		inline static TransformTp CubicSplineInterpolate(const KeyFrame<TransformTp>& a, const KeyFrame<TransformTp>& b,
		                                                 const TransformTp& aOutT, const TransformTp& bInT, float t)
		{
			...
			return (2 * x3 - 3 * x2 + 1) * a.transform +
			       (x3 - 2 * x2 + x) * tanScale * aOutT + 
			       (-2 * x3 + 3 * x2) * b.transform +
			       (x3 - x2) * tanScale * bInT;
		}
	...
	{
		inline static glm::quat DefaultTransform()
		{
		...
		inline static glm::quat CubicSplineInterpolate(const KeyFrame<glm::quat>& a, const KeyFrame<glm::quat>& b,
		                                               const glm::quat& aOutT, const glm::quat& bInT, float t)
		{
	...
	{
		inline static glm::vec3 DefaultTransform()
		{
