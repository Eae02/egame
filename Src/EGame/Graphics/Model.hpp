...
public:
	...
	MeshData<const V, const I> GetMeshData(size_t index) const
	{
		...
		data.vertices =
			std::span<const V>(static_cast<const V*>(m_meshes[index].memory.get()), m_meshes[index].numVertices);
	...
	eg::IndexType IndexType() const { return m_indexTypeE; }

	std::type_index VertexType() const { return m_vertexType; }
...
{
public:
	ModelBuilderUnformatted(
		std::type_index vertexType, uint64_t vertexSize, std::type_index indexType, uint64_t indexSize,
		IndexType indexTypeE)
		: m_vertexSize(vertexSize), m_indexSize(indexSize), m_vertexType(vertexType), m_indexType(indexType),
		  m_indexTypeE(indexTypeE)
	{
	}
	...
	std::tuple<void*, void*> AddMesh(
		uint32_t numVertices, uint32_t numIndices, std::string name, MeshAccess access = MeshAccess::All,
...
public:
	...
	MeshData<V, I> AddMesh(
		uint32_t numVertices, uint32_t numIndices, std::string name, MeshAccess access = MeshAccess::All,
		int materialIndex = -1, const Sphere* boundingSphere = nullptr, const eg::AABB* boundingAABB = nullptr)
	{
		auto [vertices, indices] = m_builder.AddMesh(
			numVertices, numIndices, std::move(name), access, materialIndex, boundingSphere, boundingAABB);
	...
	Model CreateAndReset() { return m_builder.CreateAndReset(); }
