...
void CopyBuffer(
	CommandContextHandle, BufferHandle src, BufferHandle dst, uint64_t srcOffset, uint64_t dstOffset, uint64_t size)
{
...
void Buffer::AssertRange(uint64_t begin, uint64_t length) const
{
	...
	{
		EG_PANIC(
			"Buffer range starting at " << begin << " with length " << length
