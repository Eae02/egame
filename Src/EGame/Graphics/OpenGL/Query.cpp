...
bool GetQueryResults(
	QueryPoolHandle queryPoolHandle, uint32_t firstQuery, uint32_t numQueries, uint64_t dataSize, void* data)
{
#ifdef __EMSCRIPTEN__
	return true;
#else
...
void CopyQueryResults(
	CommandContextHandle, QueryPoolHandle queryPoolHandle, uint32_t firstQuery, uint32_t numQueries,
