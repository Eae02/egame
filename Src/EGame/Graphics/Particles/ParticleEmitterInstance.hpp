...
public:
	...
	ParticleEmitterInstance() : m_id(0), m_manager(nullptr) {}

	~ParticleEmitterInstance() { Kill(); }

	ParticleEmitterInstance(ParticleEmitterInstance&& other) noexcept : m_id(other.m_id), m_manager(other.m_manager)
	{
	...
	bool Alive() const { return m_manager != nullptr; }

	float EmissionRateFactor() const { return m_emissionRateFactor; }
...
private:
	ParticleEmitterInstance(uint32_t id, ParticleManager* manager) : m_id(id), m_manager(manager) {}
