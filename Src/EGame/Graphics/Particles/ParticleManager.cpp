...
void ParticleManager::SimulateOneStep()
{
	...
	for (ParticlePage* page : m_pages)
	{
		...
		for (int i = 0; i < numAliveDiv4; i++)
		{
			...
			currentOpacityM[i] = _mm_mul_ps(
				_mm_add_ps(initialOpacityM[i], _mm_mul_ps(deltaOpacityM[i], lifeProgressM[i])), _mm_set1_ps(255.0f));
	...
	for (Emitter& emitter : m_btEmitters)
	{
		...
		while (emitter.timeSinceEmit > emitter.emissionDelay)
		{
			...
			float transformIA =
				glm::clamp((static_cast<float>(emissionsMade) * emitter.emissionDelay - oldTSE) / dt, 0.0f, 1.0f);
	...
	for (ParticlePage* page : m_pages)
	{
		for (int i = page->livingParticles - 1; i >= 0; i--)
		{
			...
			if (draw)
			{
				...
				instance.texCoord[3] =
					ToUNorm16(static_cast<float>(texVariant.y + texVariant.height) * m_texturePixelSize.y);
			}
		}
	}
...
ParticleManager::ParticlePage* ParticleManager::GetPage(const ParticleEmitterType& emitterType)
{
	auto it = std::lower_bound(
		m_pages.begin(), m_pages.end(), &emitterType,
		[&](const ParticlePage* a, const ParticleEmitterType* b) { return a->emitterType < b; });

	while (it != m_pages.end() && (*it)->emitterType == &emitterType && (*it)->livingParticles >= PARTICLES_PER_PAGE)
	{
		++it;
	}
	...
	if (m_instancesToDraw != 0)
	{
		if (m_instancesToDraw > m_deviceBufferCapacity)
		{
			...
			m_deviceBuffer = Buffer(
				BufferFlags::VertexBuffer | BufferFlags::CopyDst, m_deviceBufferCapacity * sizeof(ParticleInstance),
				nullptr);
		}
...
{
	auto it = std::lower_bound(
		m_mtEmitters.begin(), m_mtEmitters.end(), id, [&](const Emitter& a, uint32_t b) { return a.id < b; });
