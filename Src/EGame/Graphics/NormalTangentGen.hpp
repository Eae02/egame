...
std::unique_ptr<glm::vec3[], FreeDel> GenerateTangents(
	...
	GetNormalT getVertexNormal)
{
	std::unique_ptr<glm::vec3[], FreeDel> tangents(
		static_cast<glm::vec3*>(std::calloc(sizeof(glm::vec3), numVertices * 2)));
	for (size_t i = 0; i < indices.size(); i += 3)
	{
		...
		glm::vec3 d1(
			(dtc1.y * dp0.x - dtc0.y * dp1.x) * r, (dtc1.y * dp0.y - dtc0.y * dp1.y) * r,
			(dtc1.y * dp0.z - dtc0.y * dp1.z) * r);
		glm::vec3 d2(
			(dtc0.x * dp1.x - dtc1.x * dp0.x) * r, (dtc0.x * dp1.y - dtc1.x * dp0.y) * r,
			(dtc0.x * dp1.z - dtc1.x * dp0.z) * r);
...
std::unique_ptr<glm::vec3[], FreeDel> GenerateNormals(
	std::span<const IndexT> indices, size_t numVertices, GetPosT getVertexPos)
{
	...
	std::span<const IndexT> indices, size_t numVertices, GetPosT getVertexPos, GetTexCoordT getVertexTexCoord)
{
	...
	std::unique_ptr<glm::vec3[], FreeDel> tangents =
		GenerateTangents(indices, numVertices, getVertexPos, getVertexTexCoord, [&](size_t v) { return normals[v]; });
