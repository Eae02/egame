	...
	ShaderModuleHandle GetFullscreenShader(FullscreenShaderTexCoordMode texCoordMode)
	{
		...
		if (!fullScreenShaders[static_cast<int>(texCoordMode)])
		{
			...
			case FullscreenShaderTexCoordMode::NoOutput:
				shaderCode = { reinterpret_cast<const char*>(Fullscreen_TCNone_vs_glsl), sizeof(Fullscreen_TCNone_vs_glsl) };
				break;
			case FullscreenShaderTexCoordMode::NotFlipped:
				shaderCode = { reinterpret_cast<const char*>(Fullscreen_TCNoFlip_vs_glsl), sizeof(Fullscreen_TCNoFlip_vs_glsl) };
				break;
			case FullscreenShaderTexCoordMode::Flipped:
				shaderCode = { reinterpret_cast<const char*>(Fullscreen_TCFlip_vs_glsl), sizeof(Fullscreen_TCFlip_vs_glsl) };
				break;
