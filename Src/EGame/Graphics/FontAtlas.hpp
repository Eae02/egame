...
struct EG_API GlyphRange
{
	...
	GlyphRange(uint32_t _start, uint32_t _end) : start(_start), end(_end) {}

	bool operator<(const GlyphRange& other) const { return start < other.start; }

	bool operator>(const GlyphRange& other) const { return start > other.start; }

	bool operator==(const GlyphRange& other) const { return start == other.start && end == other.end; }

	bool operator!=(const GlyphRange& other) const { return !operator==(other); }
};
...
public:
	 ...
	 */
	static std::optional<FontAtlas> Render(
		const std::string& fontPath, uint32_t size, std::span<const GlyphRange> glyphRanges, int atlasWidth = -1,
		int atlasHeight = -1);
	 ...
	 */
	static std::optional<FontAtlas> Render(
		std::span<const char> data, uint32_t size, std::span<const GlyphRange> glyphRanges, int atlasWidth = -1,
		int atlasHeight = -1);
	...
	float SpaceAdvance() const { return m_spaceAdvance; }

	uint32_t AtlasWidth() const { return m_atlasData.width; }

	uint32_t AtlasHeight() const { return m_atlasData.height; }

	const uint8_t* AtlasData() const { return m_atlasData.data; }
...
private:
	...
	static std::optional<FontAtlas> FromFNTInternal(
		ReadLineCB readLineCB, LoadImageCB loadImage, std::string_view name);

	static std::optional<FontAtlas> RenderFreeType(
		void* face, int loadState, std::string_view fontName, uint32_t size, std::span<const GlyphRange> glyphRanges,
	...
	struct AtlasData
	{
		...
		~AtlasData() noexcept { std::free(data); }

		AtlasData() = default;

		AtlasData(const AtlasData& other) noexcept;

		AtlasData(AtlasData&& other) noexcept : width(other.width), height(other.height), data(other.data)
		{
			other.data = nullptr;
		}
