...
#define LOAD_FC_ENTRY_POINT(name)                                                                                      \
	fc::name = reinterpret_cast<decltype(fc::name)>(dlsym(g_fontConfigLibrary, "Fc" #name));
...
{
	inline void operator()(FcPattern* pattern) const { fc::PatternDestroy(pattern); }
};
