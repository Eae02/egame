...
#define EG_ON_INIT(callback)                                                                                           \
	static eg::detail::CallbackNodeSetter EG_CONCAT(_onInit, __LINE__){ &callback, &eg::detail::onInit };
#define EG_ON_SHUTDOWN(callback)                                                                                       \
	static eg::detail::CallbackNodeSetter EG_CONCAT(_onShutdown, __LINE__){ &callback, &eg::detail::onShutdown };
...
struct EventPage
{
	...
	EventPage(const std::type_index& _type, char* _events) : type(_type), position(0), events(_events) {}
};
...
{
public:
	EventListener() : m_page(detail::GetEventPage(std::type_index(typeid(T)))), m_position(m_page->position.load()) {}
