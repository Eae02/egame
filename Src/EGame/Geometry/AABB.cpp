...
{
	return pos.x > std::fmin(min.x, max.x) && pos.x < std::fmax(min.x, max.x) && pos.y > std::fmin(min.y, max.y) &&
	       pos.y < std::fmax(min.y, max.y) && pos.z > std::fmin(min.z, max.z) && pos.z < std::fmax(min.z, max.z);
}
