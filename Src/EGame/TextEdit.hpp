...
{
public:
	TextEdit() : m_font(nullptr) {}
	explicit TextEdit(const SpriteFont& font) : m_font(&font) {}
	...
	std::string_view Text() const { return { m_data.data(), m_data.size() }; }

	const SpriteFont* Font() const { return m_font; }

	void SetFont(const SpriteFont* font) { m_font = font; }
	...
	float TextWidth() const { return m_font->GetTextExtents(Text()).x * m_fontScale; }
